...
// Cambio de clave
let cambioClaveActualIng = "";
let cambioClaveNuevaIng  = "";
let cambioClaveConfIng   = "";
let pasosCambio = 1; // 1=actual, 2=nueva, 3=confirmar
...
const STEP_INFO = {
  ...
  stepCambiarClave:     { p:4, l:"Cambiar clave" },
...
function confirmarTarjeta() {
  ...
  if (!ced || !BASE_USUARIOS[ced]) {
    // ── TARJETA NO RECONOCIDA ──
    mostrarNoCliente(err);
    return;
  }
 
  const usuario = BASE_USUARIOS[ced];
 
  // Verificar si la cuenta está bloqueada
  if (usuario.bloqueado) {
    mostrarBloqueada(err);
    return;
  }
 
  err.textContent = "";
  usuarioPorCedula = usuario;
...
function mostrarNoCliente(errEl) {
  errEl.innerHTML = "";
  errEl.textContent = "";
 
  // Mostrar mensaje visual debajo del campo
  const existente = document.getElementById("noClienteMsg");
  if (existente) existente.remove();
 
  const div = document.createElement("div");
  div.id = "noClienteMsg";
  div.className = "no-cliente-box";
  div.innerHTML = `
    <div class="no-cliente-icon">🏦</div>
    <div class="no-cliente-titulo">Usted no es cliente</div>
    <div class="no-cliente-msg">
      El número ingresado no está registrado en nuestro sistema.<br><br>
      <strong>¿Desea abrir una cuenta?</strong><br>
      Diríjase a una oficina de <strong>Uniremington</strong> y un asesor le ayudará a aperturar su cuenta sin costo.
    </div>
    <div class="no-cliente-badge">🏢 Oficinas Uniremington — Lunes a Viernes 8am–5pm</div>`;
  errEl.parentNode.insertBefore(div, errEl.nextSibling);
  scr("No registrado", "El número no corresponde a ningún cliente.", "error");
}
 
function mostrarBloqueada(errEl) {
  errEl.innerHTML = "";
  errEl.textContent = "";
 
  const existente = document.getElementById("bloqueadaMsg");
  if (existente) existente.remove();
 
  const div = document.createElement("div");
  div.id = "bloqueadaMsg";
  div.className = "bloqueada-box";
  div.innerHTML = `
    <div class="bloqueada-icon">🔒</div>
    <div class="bloqueada-titulo">Cuenta bloqueada por seguridad</div>
    <div class="bloqueada-msg">
      Su cuenta ha sido bloqueada por superar el número de intentos permitidos.<br><br>
      Para desbloquearla, diríjase a una oficina de <strong>Uniremington</strong> con su documento de identidad.
    </div>
    <div class="bloqueada-badge">📞 Línea de atención: 01 8000 123 456</div>`;
  errEl.parentNode.insertBefore(div, errEl.nextSibling);
  scr("Cuenta bloqueada", "Diríjase a una oficina Uniremington.", "error");
}
 
function limpiarMensajesExtra() {
  ["noClienteMsg","bloqueadaMsg"].forEach(id => {
...
function confirmarCedula() {
  ...
  if (!u) {
    // ── CÉDULA NO REGISTRADA → No es cliente ──
    scr("No registrado", "La cédula ingresada no corresponde a ningún cliente.", "error");
    flashErrCed();
    const errEl = document.getElementById("cedulaError");
    errEl.textContent = "";
    limpiarMensajesExtra();
    const div = document.createElement("div");
    div.id = "noClienteMsg";
    ...
    div.innerHTML = `
      ...
      <div class="no-cliente-titulo">Usted no es cliente</div>
      <div class="no-cliente-msg">
        La cédula ingresada no está registrada en nuestro sistema.<br><br>
        <strong>¿Desea abrir una cuenta?</strong><br>
        Diríjase a una oficina de <strong>Uniremington</strong> y un asesor le ayudará a aperturar su cuenta sin costo.
      </div>
      <div class="no-cliente-badge">🏢 Oficinas Uniremington — Lunes a Viernes 8am–5pm</div>`;
  ...
  if (u.bloqueado) {
    ...
    div.innerHTML = `
      ...
      <div class="bloqueada-titulo">Cuenta bloqueada por seguridad</div>
      <div class="bloqueada-msg">
        Su cuenta ha sido bloqueada. Diríjase a una oficina de <strong>Uniremington</strong> con su documento de identidad para desbloquearla.
      </div>
      <div class="bloqueada-badge">📞 Línea de atención: 01 8000 123 456</div>`;
    errEl.parentNode.insertBefore(div, errEl.nextSibling);
    cedulaIng = "";
...
function confirmarPin() {
  ...
  if (!usuarioPorCedula || usuarioPorCedula.pin !== pinIng) {
    ...
    if (intentosFallidos >= 3) {
      // ── CUENTA BLOQUEADA POR 3 INTENTOS ──
      if (usuarioPorCedula && BASE_USUARIOS[usuarioPorCedula.cedula]) {
        BASE_USUARIOS[usuarioPorCedula.cedula].bloqueado = true;
      }
      scr("Cuenta bloqueada", "Cuenta bloqueada por seguridad. Contacte al banco.", "error");
      flashErrPin(); pinIng = "";
      document.getElementById("pinError").textContent = "";
 
      // Mostrar mensaje de cuenta bloqueada en el paso de PIN
      ...
      div.innerHTML = `
        ...
        <div class="bloqueada-titulo">Cuenta bloqueada por seguridad</div>
        <div class="bloqueada-msg">
          Se han agotado los 3 intentos permitidos.<br>
          Su cuenta ha sido bloqueada automáticamente.<br><br>
          Diríjase a una oficina de <strong>Uniremington</strong> con su documento de identidad para desbloquearla.
        </div>
        <div class="bloqueada-badge">📞 Línea de atención: 01 8000 123 456</div>`;
      errEl.parentNode.insertBefore(div, errEl.nextSibling);
...
/* ══════════════════════════════════════════════
   CAMBIAR CLAVE — LÓGICA COMPLETA
   ══════════════════════════════════════════════ */
function iniciarCambioClave() {
  cambioClaveActualIng = "";
  cambioClaveNuevaIng  = "";
  cambioClaveConfIng   = "";
  pasosCambio = 1;
 
  updDotsCambio("dca", cambioClaveActualIng);
  updDotsCambio("dcn", cambioClaveNuevaIng);
  updDotsCambio("dcc", cambioClaveConfIng);
 
  document.getElementById("cambioClaveActualError").textContent = "";
  document.getElementById("cambioClaveNuevaError").textContent  = "";
  document.getElementById("cambioClaveConfirmarError").textContent = "";
 
  document.getElementById("cambioClaveActual").style.display    = "block";
  document.getElementById("cambioClaveNueva").style.display     = "none";
  document.getElementById("cambioClaveConfirmar").style.display = "none";
  document.getElementById("cambioClaveSubtitulo").textContent   = "Paso 1: Ingrese su clave actual";
 
  scr("Cambiar clave", "Ingrese su clave actual para continuar.");
  showStep("stepCambiarClave");
}
 
function updDotsCambio(prefix, valor) {
  for (let i = 0; i < 4; i++) {
    const el = document.getElementById(prefix + i);
    if (el) el.className = "dot" + (i < valor.length ? " filled" : "");
  }
}
function flashDotsCambio(prefix) {
  for (let i = 0; i < 4; i++) {
    const el = document.getElementById(prefix + i);
    if (el) el.className = "dot error-flash";
  }
}
 
/* Numpad clave actual */
function npCA(d) {
  if (cambioClaveActualIng.length < 4) {
    cambioClaveActualIng += d;
    document.getElementById("cambioClaveActualError").textContent = "";
    updDotsCambio("dca", cambioClaveActualIng);
  }
}
function ndCA() {
  if (cambioClaveActualIng.length > 0) {
    cambioClaveActualIng = cambioClaveActualIng.slice(0, -1);
    document.getElementById("cambioClaveActualError").textContent = "";
    updDotsCambio("dca", cambioClaveActualIng);
  }
}
function confirmarClaveActual() {
  if (cambioClaveActualIng.length < 4) {
    document.getElementById("cambioClaveActualError").textContent = "Ingrese los 4 dígitos de su clave actual.";
    return;
  }
  if (cambioClaveActualIng !== usuarioActual.pin) {
    document.getElementById("cambioClaveActualError").textContent = "Clave actual incorrecta.";
    flashDotsCambio("dca");
    setTimeout(() => updDotsCambio("dca", ""), 600);
    cambioClaveActualIng = "";
    scr("Clave incorrecta", "La clave actual no coincide.", "error");
    return;
  }
  // Avanzar a paso 2
  pasosCambio = 2;
  document.getElementById("cambioClaveActual").style.display    = "none";
  document.getElementById("cambioClaveNueva").style.display     = "block";
  document.getElementById("cambioClaveConfirmar").style.display = "none";
  document.getElementById("cambioClaveSubtitulo").textContent   = "Paso 2: Ingrese su nueva clave";
  document.getElementById("cambioClaveNuevaError").textContent  = "";
  cambioClaveNuevaIng = "";
  updDotsCambio("dcn", cambioClaveNuevaIng);
  scr("Nueva clave", "Ingrese su nueva clave de 4 dígitos.");
}
 
/* Numpad nueva clave */
function npCN(d) {
  if (cambioClaveNuevaIng.length < 4) {
    cambioClaveNuevaIng += d;
    document.getElementById("cambioClaveNuevaError").textContent = "";
    updDotsCambio("dcn", cambioClaveNuevaIng);
  }
}
function ndCN() {
  if (cambioClaveNuevaIng.length > 0) {
    cambioClaveNuevaIng = cambioClaveNuevaIng.slice(0, -1);
    document.getElementById("cambioClaveNuevaError").textContent = "";
    updDotsCambio("dcn", cambioClaveNuevaIng);
  }
}
function confirmarClaveNueva() {
  if (cambioClaveNuevaIng.length < 4) {
    document.getElementById("cambioClaveNuevaError").textContent = "Ingrese los 4 dígitos de la nueva clave.";
    return;
  }
  if (cambioClaveNuevaIng === cambioClaveActualIng) {
    document.getElementById("cambioClaveNuevaError").textContent = "La nueva clave no puede ser igual a la actual.";
    flashDotsCambio("dcn");
    setTimeout(() => updDotsCambio("dcn", ""), 600);
    cambioClaveNuevaIng = "";
    return;
  }
  // Avanzar a paso 3
  ...
  document.getElementById("cambioClaveActual").style.display    = "none";
  document.getElementById("cambioClaveNueva").style.display     = "none";
  document.getElementById("cambioClaveConfirmar").style.display = "block";
  document.getElementById("cambioClaveSubtitulo").textContent   = "Paso 3: Confirme su nueva clave";
  document.getElementById("cambioClaveConfirmarError").textContent = "";
  cambioClaveConfIng = "";
  updDotsCambio("dcc", cambioClaveConfIng);
  scr("Confirmar clave", "Ingrese nuevamente la nueva clave.");
}
 
/* Numpad confirmación clave */
function npCC(d) {
  if (cambioClaveConfIng.length < 4) {
    cambioClaveConfIng += d;
    document.getElementById("cambioClaveConfirmarError").textContent = "";
    updDotsCambio("dcc", cambioClaveConfIng);
  }
}
function ndCC() {
  if (cambioClaveConfIng.length > 0) {
    cambioClaveConfIng = cambioClaveConfIng.slice(0, -1);
    document.getElementById("cambioClaveConfirmarError").textContent = "";
    updDotsCambio("dcc", cambioClaveConfIng);
  }
}
function confirmarCambioFinal() {
  if (cambioClaveConfIng.length < 4) {
    document.getElementById("cambioClaveConfirmarError").textContent = "Ingrese los 4 dígitos de confirmación.";
    return;
  }
  if (cambioClaveConfIng !== cambioClaveNuevaIng) {
    document.getElementById("cambioClaveConfirmarError").textContent = "Las claves no coinciden. Intente de nuevo.";
    flashDotsCambio("dcc");
    setTimeout(() => updDotsCambio("dcc", ""), 600);
    cambioClaveConfIng = "";
    scr("No coinciden", "Las claves ingresadas no son iguales.", "error");
    return;
  }
 
  // ── APLICAR CAMBIO DE CLAVE ──
  const nuevaClave = cambioClaveNuevaIng;
  usuarioActual.pin = nuevaClave;
  if (BASE_USUARIOS[usuarioActual.cedula]) {
    BASE_USUARIOS[usuarioActual.cedula].pin = nuevaClave;
  }
  // También actualizar el usuario por cédula si está en memoria
  if (usuarioPorCedula && usuarioPorCedula.cedula === usuarioActual.cedula) {
    usuarioPorCedula.pin = nuevaClave;
  }
 
  scr("Clave actualizada", "Su clave ha sido cambiada exitosamente.", "success");
 
  // Mostrar comprobante de cambio de clave
  ...
  document.getElementById("ticketFinal").innerHTML = `
    <div class="ticket-header" style="background:#0F6E56;">
      <div class="t-logo">Cajero Uniremington • Comprobante</div>
      <div class="t-title">Cambio de clave exitoso</div>
      <div class="t-ref">${fecha}</div>
    </div>
    <div class="ticket-body">
      <div class="t-section-label">Detalle del cambio</div>
      <div class="t-row"><span class="lbl">Titular</span><span class="val">${usuarioActual.nombre}</span></div>
      <div class="t-row"><span class="lbl">Cédula</span><span class="val">${usuarioActual.cedula}</span></div>
      <div class="t-row"><span class="lbl">Cuenta</span><span class="val">${usuarioActual.numeroCuenta}</span></div>
      <div class="t-row"><span class="lbl">Estado</span><span class="val green">✔ Clave actualizada</span></div>
      <div class="t-section-label">Seguridad</div>
      <div class="t-row"><span class="lbl">Recuerde</span><span class="val" style="font-size:11px;text-align:right;">No comparta su clave con nadie</span></div>
    </div>
...
function reiniciar() {
  ...
  saldo = 0; saldoAhorro = 0; saldoCorriente = 0; tipo = "Ahorro";
  tipoCajero = ""; tarjetaIng = ""; servicioSelPago = null; intentosFallidos = 0;
  cambioClaveActualIng = ""; cambioClaveNuevaIng = ""; cambioClaveConfIng = ""; pasosCambio = 1;
